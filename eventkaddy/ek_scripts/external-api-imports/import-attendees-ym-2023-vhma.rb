...
def return_attendee_attributes_hash(attendee, event_id)
  ...
  state = nil
  if attendee["strLocation"].present?
    state = attendee["strLocation"]
  else
    start = attendee["state"]
  end
	...
	{ 
		...
		password:					"VHMA2023",
		custom_filter_2:  attendee["people_id"],
		mobile_phone:     attendee["strPhone"],
    business_phone:   attendee["Custom_AddressLine1"],
    address:          attendee["strAddress2"],
    postal_code:      attendee["strPostalCode"]
	}
end
...
HOST            = 'https://staging-proxyb.eventkaddy.net'
...
attendees_url = HOST + '/attendeedata_vhma_your_membership_2023?proxy_key='+MERCURY_CREDS+'&refresh='+refresh+'&attendee_type='+attendee_type
