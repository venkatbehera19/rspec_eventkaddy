...
def return_sessions_attribute_hash(session)
  ...
  if (accreditation_field.length > 0 && accreditation_field != "credits and type") then
    acceditation_value = accreditation_field;
  else
...
def return_full_audience_name(track_desc_org)
  track_desc = track_desc_org.to_s.tr("<br>", "")
  case track_desc
  when "(A&I)"
...
def collect_session_tags_from_data(session, ar_session)
  ...
  unless track.blank? && subtrack.blank?
    ...
    if subtrack.blank? && poster.blank? && audience.blank?
      line_break = ''
    else
      line_break = "||"
    end
  ...
  ar_session.update!(track_subtrack:track_subtrack, custom_filter_1:custom_filter_1, custom_filter_2:custom_filter_2, tags_safeguard:tags_safeguard, premium_access:premium_access)
...
def return_speakers_attribute_hash(speaker)
  ...
  speaker_attrs = {event_id:@event_id,speaker_code:speaker_code}
end
...
def create_session_speaker_associations(ar_session, ar_speaker, speaker)
  ...
  session_speaker_attrs = {session_id:ar_session.id,speaker_id:ar_speaker.id, unpublished:unpublished}
...
planstone_url = 'https://static.planion.com/feed/v1/sessions?where=%7B%22account%22%3A%22BCBSNS%22%2C%22conference%22%3A%22NM20%22%7D'
