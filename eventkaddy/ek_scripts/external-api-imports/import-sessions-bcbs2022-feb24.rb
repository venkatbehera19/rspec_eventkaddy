...
if (accreditation_field.present? && accreditation_field != "credits and type") then
  ...
  if (session["custom3name"] == "Learning Objectives")
    ...
    if (session["learning_objective1"] != nil) then
      learning_objective = session["learning_objective1"] 
 	  learning_objective += "<br><br>"
    end
  
  if (session["learning_objective2"] != nil) then
      learning_objective += session["learning_objective2"]
      learning_objective += "<br><br>"
    end
  if (session["learning_objective3"] != nil) then
      learning_objective += session["learning_objective3"]
      learning_objective += "<br><br>"
    end

  #if (session["learning_objective4"] != nil) then
  #    learning_objective += session["learning_objective4"]
  #    learning_objective += "<br><br>"
  #  end

 # if (session["learning_objective5"] != nil) then
  #    learning_objective += session["learning_objective5"]
   #   learning_objective += "<br><br>"
  #  end
  #if (session["learning_objective6"] != nil) then
   #   learning_objective += session["learning_objective6"]
   #   learning_objective += "<br><br>"
   # end
  #if (session["learning_objective7"] != nil) then
   #   learning_objective += session["learning_objective7"]
    #  learning_objective += "<br><br>"
   # end
  #if (session["learning_objective8"] != nil) then
   #   learning_objective += session["learning_objective8"]
   #   learning_objective += "<br><br>"
   # end
  #if (session["learning_objective10"] != nil) then
   #   learning_objective += session["learning_objective9"]
    #  learning_objective += "<br><br>"
    #end
  #if (session["learning_objective9"] != nil) then
   #   learning_objective += session["learning_objective10"]
   # end
...
def collect_session_tags_from_data(session, ar_session)
  ...
  ar_session.update!(track_subtrack:track_subtrack, custom_filter_1:custom_filter_1, custom_filter_2:custom_filter_2, custom_filter_3:custom_filter_3, promotion:promotion, tags_safeguard:tags_safeguard, premium_access:premium_access)
...
def collect_session_tags_from_data_v2(session, ar_session)
  ...
  if session['tracks'].is_a?(Array)
    ...
    tracks_array.each do |track|
      ...
      track_desc = return_full_audience_name(track_desc)
      tags_array.push(['Audiences', track_name, track_desc])
    end
  end
  tags_array
end
