...
begin
	...
	oo.sheets.each do |sheet|
		...
		2.upto(oo.last_row) do |row|
			...
			if (is_number(colvals[19])) then
				capacity = colvals[19].to_i.to_s
			else
				capacity = colvals[19]			
			end
			...
			session_sponsors = colvals[23].split('##').map { |a| a.strip } #comma separated list of company names
			...
			session_attrs = {event_id:event_id,session_code:session_code,title:session_title,description:session_description,
				...
				price:price,track_subtrack:session_track_subtrack }			
			...
			if (program_type.length==1) then
				session.program_type_id = program_type.id
			else
