...
job.start {
  ...
  populate_hash_for_delta = ->(hash, model) {
    hash.merge({
      ...
      attendee_type:        model.attendee_type&.name,
  ...
  oo.sheets.each do |sheet|
    ...
    #  temp_photo_filename = valueFor[@temp_photo_filename_col]
      ...
      attendee_type_val   = valueFor[@attendee_type_col]
    ...
    premium_member = valueFor[@premium_member_col]
      ...
      attendee_type_id = AttendeeType.find_by(name: attendee_type_val)&.id
      attendee_type_id = AttendeeType.find_by(id: attendee_type_val)&.id if attendee_type_id.blank?

      attendee_type_id = AttendeeType.find_by(name: 'Standard Attendee')&.id if attendee_type_val.blank?

      custom_error = "Attendee Type of #{attendee_type_val} not found" if attendee_type_id.blank? && !attendee_type_val.blank?
      ...
      if attendees.blank?
        ...
        attendee.assign_attributes(attendee_attrs)
      else
        puts "problem with attendee with email #{email} account_code #{registration_id}"
      end

      if custom_error.blank? && attendee.save
        ...
        delta_report.after = populate_hash_for_delta[ {}, attendee]
        delta_report.compare({status: 'failed', message: attendee.errors.full_messages.push(custom_error).join(", ")})
